 ...
 */
 ...
 (function(window) {
   ...
   initLayoutEngine = function() {
     ...
     onresize = function() {
       ...
       generateRuleCode = function(rule) {
         ...
         declarations = [];
         ruleCss = (map(rule.prelude, function(o) {
           if (o.toSource != null) {
             return o.toSource();
           } else {
             return '';
         ...
         for (_i = 0, _len = _ref.length; _i < _len; _i++) {
           ...
           for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
             token = _ref1[_j];
             if (token.tokenType === 'DIMENSION' && (token.unit === 'vmin' || token.unit === 'vh' || token.unit === 'vw')) {
               ruleCss += "" + (Math.floor(token.value * dims[token.unit])) + "px";
             } else {
               ruleCss += token.toSource();
             }
           }
           ruleCss += ";";
         }
                 ...
                 for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
                   ...
                   if (t.name === '(') {
                     ...
                     for (_k = 0, _len2 = _ref2.length; _k < _len2; _k++) {
                       t1 = _ref2[_k];
                       source = t1.toSource != null ? t1.toSource() : '';
     ...
     for (_i = 0, _len = links.length; _i < _len; _i++) {
       ...
       (function (i) {
         ajax(i.href, function(cssText) {
           ...
           var sheet = parseAStylesheet(cssText);
   ...
   function isDev (env) {
     return (env === 'dev' || env === 'DEV' || env === 'develop' || env== 'DEVELOP')
   }

   if (!browserSupportsUnitsNatively() || isDev(window.VMINPOLY_ENV)) {
     initLayoutEngine();
   }   
